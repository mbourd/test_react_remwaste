...
export default function SkipCard({ title, price, hirePeriod, allowedOnRoad, children, isSelected }) {
  return (
    <div className={`backdrop-blur-[10px] bg-white relative overflow-hidden rounded-2xl shadow-xl border hover:ring-2 hover:ring-blue-400 transition-all duration-300 p-6 space-y-4 hover:bg-opacity-100 ${allowedOnRoad ? 'border-blue-500' : 'border-red-400'} ${isSelected ? 'bg-opacity-100 font-bold':'bg-opacity-55'}`}>
